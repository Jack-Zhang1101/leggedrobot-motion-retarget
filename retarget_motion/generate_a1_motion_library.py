"""Generate an A1-compatible motion library."""
...
DEFAULT_OUTPUT_DIR = REPO_ROOT / "motion_imitation" / "data" / "motions_a1"
...
def _generate_raw_motion(spec):
  ...
  return retarget_core.retarget_joint_data("a1", joint_pos_data, gui=False)


def _generate_adapted_motion(spec):
  return motion_adaptation.load_and_adapt_motion_file(spec["source_motion_path"])
...
def _generate_motion(name, generated_cache):
  ...
  spec = a1_motion_sources.MOTION_SPECS[name]
  mode = spec["mode"]
  if mode == a1_motion_sources.RAW:
    frames = _generate_raw_motion(spec)
  elif mode == a1_motion_sources.DERIVED:
    base_frames = _generate_motion(spec["source_motion"], generated_cache)
    frames = a1_motion_sources.reverse_motion_frames(base_frames)
  elif mode == a1_motion_sources.ADAPTED:
...
def generate_library(output_dir=DEFAULT_OUTPUT_DIR, motion_names=None):
  ...
  selected = motion_names or a1_motion_sources.ordered_motion_names()
  generated_cache = {}
  provenance = {}
...
def main():
  ...
  parser.add_argument(
      ...
      help="Output directory for generated A1 motions.")
  parser.add_argument(
      "--motion",
      action="append",
      dest="motions",
