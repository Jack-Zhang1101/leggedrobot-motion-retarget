"""Generate a Sizu-compatible motion library."""
...
if __package__ in (None, ""):
  ...
  from retarget_motion import sizu_motion_sources
else:
  from motion_imitation.utilities import motion_data
  from retarget_motion import motion_adaptation
  from retarget_motion import retarget_core
...
DEFAULT_OUTPUT_DIR = REPO_ROOT / "motion_imitation" / "data" / "motions_sizu"
...
def _generate_raw_motion(spec):
  ...
  return retarget_core.retarget_joint_data("sizu", joint_pos_data, gui=False)


def _generate_adapted_motion(spec):
  return motion_adaptation.load_and_adapt_motion_file(
      spec["source_motion_path"],
      target_robot_name="sizu",
      target_root_height_offset=spec.get("adapt_root_height_offset"),
      leg_scale_multiplier=spec.get("adapt_leg_scale_multiplier", 1.0),
      joint_smoothing_window=spec.get("adapt_joint_smoothing_window"))
...
def _generate_motion(name, generated_cache):
  ...
  spec = sizu_motion_sources.MOTION_SPECS[name]
  mode = spec["mode"]
  if mode == sizu_motion_sources.RAW:
    frames = _generate_raw_motion(spec)
  elif mode == sizu_motion_sources.DERIVED:
    base_frames = _generate_motion(spec["source_motion"], generated_cache)
    frames = sizu_motion_sources.reverse_motion_frames(base_frames)
  elif mode == sizu_motion_sources.ADAPTED:
...
def generate_library(output_dir=DEFAULT_OUTPUT_DIR, motion_names=None):
  ...
  selected = motion_names or sizu_motion_sources.ordered_motion_names()
  generated_cache = {}
  provenance = {}
  ...
  for name in selected:
    ...
    provenance[name] = {
        ...
        "adapt_leg_scale_multiplier": spec.get("adapt_leg_scale_multiplier"),
        "adapt_joint_smoothing_window": spec.get("adapt_joint_smoothing_window"),
...
def main():
  ...
  parser.add_argument(
      ...
      help="Output directory for generated Sizu motions.")
  parser.add_argument(
      "--motion",
      action="append",
      dest="motions",
